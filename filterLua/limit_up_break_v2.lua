-- @id: 3
-- @name: 涨停突破策略（含十字星与SMA趋势筛选）
-- @description: 识别涨停突破的股票，结合量价关系进行分析，筛选出现十字星且SMA6和SMA18趋势向上且SMA6>SMA18的股票
...
-- 技术指标参数
local DOJI_THRESHOLD = 0.1                -- 十字星实体/影线比例阈值
local SMA_SHORT_PERIOD = 6                -- 短期均线周期
local SMA_LONG_PERIOD = 18                -- 长期均线周期
local MIN_KLINE_DAYS = 18                 -- 最少需要的K线数据天数
...
-- 判断是否出现十字星
function isDoji(kdata)
    local body = math.abs(kdata.close - kdata.open)
    local range = kdata.high - kdata.low
    return body <= range * DOJI_THRESHOLD
end

-- 计算SMA
function calculateSMA(kdata_list, period)
    local sma = {}
    -- 确保有足够的数据计算均线
    if #kdata_list < period then
        return nil
    end
    ...
    -- 计算每个位置的SMA
    for i = period, #kdata_list do
        local sum = 0
        for j = i - period + 1, i do
            sum = sum + kdata_list[j].close
        end
        sma[i] = sum / period
    end
    return sma
...
-- 判断SMA趋势是否向上
function isSMAUpTrend(sma, index)
    -- 确保有足够的数据判断趋势
    if not sma or not sma[index] or not sma[index - 1] then
        return false
    end
...
function processStocksParallel(indices, batchSize)
    ...
    for i, index in ipairs(indices) do
        ...
        local sma6 = nil
        local sma18 = nil
        local isDojiPresent = false
        local isSMA6Up = false
        local isSMA18Up = false
        local isSMA6AboveSMA18 = false
        ...
        -- 确保有足够的数据计算技术指标
        if #kdata < MIN_KLINE_DAYS then
            log(string.format("数据不足以计算技术指标: %d天 < %d天", #kdata, MIN_KLINE_DAYS))
            goto continue
        end
        ...
        -- 计算SMA6和SMA18
        sma6 = calculateSMA(kdata, SMA_SHORT_PERIOD)
        sma18 = calculateSMA(kdata, SMA_LONG_PERIOD)
        
        -- 检查是否成功计算了均线
        if not sma6 or not sma18 then
            log("计算均线失败")
            goto continue
        end
        
        -- 判断是否出现十字星
        lastKData = kdata[#kdata]
        isDojiPresent = isDoji(lastKData)
        
        -- 判断SMA6和SMA18趋势是否向上且SMA6 > SMA18
        isSMA6Up = isSMAUpTrend(sma6, #kdata)
        isSMA18Up = isSMAUpTrend(sma18, #kdata)
        isSMA6AboveSMA18 = sma6[#kdata] and sma18[#kdata] and sma6[#kdata] > sma18[#kdata]
        
        log(string.format("5. 十字星: %s", isDojiPresent and "是" or "否"))
        log(string.format("6. SMA6趋势: %s", isSMA6Up and "向上" or "向下"))
        log(string.format("7. SMA18趋势: %s", isSMA18Up and "向上" or "向下"))
        log(string.format("8. SMA6 > SMA18: %s", isSMA6AboveSMA18 and "是" or "否"))
        
        if #limitUps >= MIN_LIMIT_UP_COUNT and #limitUps <= MAX_LIMIT_UP_COUNT 
           and limitUps[1].isBreakthrough 
           and validRetracement
           and checkTurnoverRate(kdata[#kdata])
           and isDojiPresent
           and isSMA6Up
           and isSMA18Up
           and isSMA6AboveSMA18 then
            ...
            api.sendSignal(
                ...
                lastKData.volume,
                lastKData.amount,
                string.format("近90天内出现%d次涨停，最新涨停突破前期高点且回调企稳，换手率%.1f%%，出现十字星且SMA6>SMA18", #limitUps, lastKData.turnover)
            )
            ...
            log(string.format("\n技术指标分析:"))
            log(string.format("十字星形态: %s", isDojiPresent and "有" or "无"))
            if sma6[#kdata] and sma18[#kdata] then
                log(string.format("SMA6: %.2f", sma6[#kdata]))
                log(string.format("SMA18: %.2f", sma18[#kdata]))
            end
            log(string.format("均线趋势: %s", (isSMA6Up and isSMA18Up) and "向上" or "向下"))
        end
